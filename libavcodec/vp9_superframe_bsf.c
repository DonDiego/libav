...
static int vp9_superframe_filter(AVBSFContext *ctx, AVPacket *out)
{
    ...
    if ((res = bitstream_init8(&bc, in->data, in->size)) < 0)
        goto done;

    bitstream_read(&bc, 2); // frame marker
    profile  = bitstream_read_bit(&bc);
    profile |= bitstream_read_bit(&bc) << 1;
    if (profile == 3)
        profile += bitstream_read_bit(&bc);
    ...
    if (bitstream_read_bit(&bc)) {
        ...
        bitstream_read_bit(&bc); // keyframe
        invisible = !bitstream_read_bit(&bc);
    }
